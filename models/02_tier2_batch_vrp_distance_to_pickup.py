"""
Batch VRP Model (Tier 2)
Clustering-Based Many-to-One Assignment with Bundling
Cost Function: Distance to Pickup

This model uses K-Means clustering to group orders geographically,
then assigns multiple orders to a single courier (bundling).
"""
...
MODEL_NAME = "02_tier2_batch_vrp_distance_to_pickup"
...
print("="*80)
print("BATCH VRP MODEL (TIER 2) - Clustering + Bundling")
print("="*80)
...
def run_simulation_with_viz():
    ...
    assignment_strategy = Tier2BatchVRP(cost_function)
    print(f"Assignment strategy: Tier 2 Batch VRP (Clustering + Bundling)")
    ...
    for dispatch_idx, dispatch_time in enumerate(dispatch_times):
        ...
        # --- E. PROCESS BUNDLED ASSIGNMENTS & REJECTIONS ---
        # Group assignments by courier (bundling)
        courier_bundles = {}
        for order, courier, cost in proposed_assignments:
            c_id = courier['courier_id']
            if c_id not in courier_bundles:
                courier_bundles[c_id] = {'courier': courier, 'assignments': []}
            courier_bundles[c_id]['assignments'].append((order, courier, cost))
        ...
        for c_id, bundle_data in courier_bundles.items():
            ...
            if bundle_accepted:
                ...
                update_courier_after_assignment(
                    c_id, courier_states, dispatch_time,
                    delivery_location, bundle_task_duration, timeline_logger
                )

        n_accepted = len(accepted_assignments)
        n_rejected = len(rejected_assignments)
        avg_bundle_size = np.mean(bundle_sizes) if bundle_sizes else 0
        max_bundle_size = max(bundle_sizes) if bundle_sizes else 0

        print(f"  Assignments made: {n_accepted}/{n_orders}")
        print(f"  Bundles created: {len(courier_bundles)}")
        print(f"  Avg bundle size: {avg_bundle_size:.2f} orders/courier")
        print(f"  Max bundle size: {max_bundle_size} orders")
        ...
        # Show sample bundles
        if len(courier_bundles) > 0:
            print(f"  Sample bundles:")
            for i, (c_id, bundle_data) in enumerate(list(courier_bundles.items())[:3]):
                bundle = bundle_data['assignments']
                order_ids = [o[0]['order_id'] for o in bundle]
                print(f"    Courier {c_id}: {len(bundle)} orders - {order_ids[:5]}{'...' if len(order_ids) > 5 else ''}")
    ...
    print("\n" + "="*80)
    print("BATCH VRP MODEL COMPLETE (TIER 2)")
    print("="*80)

    print("\nKEY FINDINGS:")
    print("- Tier 2: Many-to-One assignment with bundling")
    print("- Clusters orders using K-Means, assigns bundles to couriers")
    print("- Cost = Euclidean distance from courier to cluster centroid")
    print("- Task duration scales with bundle size (AVERAGE_TASK_DURATION * num_orders)")
    print("- Stateful simulation with calibrated physics")
    print("- Bundle-level rejection model (13.11% rejection rate)")
